...
export class SupportsService {
    ...
    async create(createSupportDTO: CreateSupportDTO): Promise<Support> {
        const existingSupport:
            | Support
            | undefined = await this.supportRepository.findOne({
            from: createSupportDTO.from,
            to: createSupportDTO.to,
        });
        if (!existingSupport) {
    ...
    async findUserSupported(
        findSupportsUserDTO: FindSupportsUserDTO,
    ): Promise<User[]> {
        const supports: Support[] = await this.supportRepository.find({
            from: findSupportsUserDTO,
        });

        return supports.map((e) => e.to);
    }

    async findUserSupporters(
        findSupportsUserDTO: FindSupportsUserDTO,
    ): Promise<User[]> {
        const supports: Support[] = await this.supportRepository.find({
            to: findSupportsUserDTO,
        });
